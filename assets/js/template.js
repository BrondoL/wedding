...
var fn_rsvp_init = function () {
    ...
    var onSuccess = function (res) {
        ...
        if (res.rsvp_content && res.rsvp_content != "") {
            ...
            // RSVP Status
            if (
                $('input[type="radio"][name="rsvp_status"]:checked').length == 0
            ) {
    ...
    function showKadoModal(
        ...
        var onSuccess = function (res) {
            if (res.modal !== "") {
                openModal(res.modal);
                var formatPrice =
                    "Rp " + parseFloat(KadoPrice).toLocaleString("id-ID");
                ...
                KadoModal.find(".note-kado").text(
                    "Sudah ada yang membeli produk sebanyak " +
                        KadoDibeli +
                        " produk."
                );
    ...
    function sendKado() {
        ...
        $(document).on("submit", "form#frmBuyGift", function (e) {
            ...
            var onSuccess = function (res) {
                ...
                if (res.message)
                    showAlert({ type: "success", caption: res.message });
                closeModal();
            };
